...
describe('parser/util/LinkedList', () => {
  ...
  it('should be persistent', () => {
    ...
    assert.strictEqual(list3.head(), 'hello', 'list 3 head common');
    assert.strictEqual(list3.tail().head(), 'not common',
      'list 3 parent not common');
    assert.strictEqual(list4.head(), 'world', 'list 4 head world');
    assert.strictEqual(list4.tail().head(), 'not common',
      'list 3 parent is missing');
  });
});
